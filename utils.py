...
def random_crop(image, boxes, labels, difficulties):
    ...
    original_h = image.size(1) # [0,1] [96,h,w] tensor
    original_w = image.size(2)
...
def resize(image, boxes, dims=(300, 300), return_percent_coords=True):
    ...
    new_boxes = boxes / old_dims  # percent coordinates，将bbox归一化
...
def transform(image, boxes, labels, difficulties, split):
    ...
    mean = [0.5] * 96  # Asumiendo que 0.5 es la media para todos los canales
    std = [0.2] * 96   # Asumiendo que 0.2 es la desviación estándar para todos los canales

    # Convertir imagen a tensor y asegurarse de que tiene dimensiones (C, H, W)
    new_image = torch.from_numpy(image).permute(2, 0, 1).float() / 255.

    # Asegurar que el tensor tiene las dimensiones correctas
    if new_image.dim() != 3 or new_image.shape[0] != 96:
        raise ValueError(f"Expected image with 96 channels, got shape {new_image.shape}")
...
def tiff_to_numpy(tiff_image_name):
    # Usa tifffile para leer la imagen directamente
    img = tifffile.imread(tiff_image_name)
    
    # Si la imagen tiene la forma (96, height, width), reordénala a (height, width, 96)
    if img.shape[0] == 96 and img.ndim == 3:
        img = np.transpose(img, (1, 2, 0))
    
    # Verifica si la imagen ahora tiene la dimensión correcta
    if img.ndim == 3 and img.shape[2] == 96:
        return img
    else:
        raise ValueError(f'Unexpected image shape: {img.shape}, expected 96 channels in the third dimension.')
...
def tiff_to_numpy(tiff_image_name, target_size=(467, 348)):  # altura x anchura
    img = tifffile.imread(tiff_image_name)
    ...
    # Usa OpenCV para redimensionar la imagen
    ...
    if img.ndim == 3 and img.shape[2] == 96:
        ...
        raise ValueError(f'Unexpected image shape: {img.shape}, expected third dimension to have 96 channels.')
